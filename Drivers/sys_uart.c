...
void sys_UART_TxString(uint8_t v_uartChannel_u8, char *ptr_stringPointer_u8)
{
...
uint8_t sys_UART_RxString(uint8_t v_uartChannel_u8, char *ptr_string)
{
    char ch;
    uint8_t len = 0;
    while(1)
    {
        ...
        {   
		  if(len!=0)         /* Wait till atleast 1 char is received */              
           { 
		       ptr_string[len]='\n';           //once enter key is pressed null terminate the string and break the loop 			
						 ptr_string[len]='\n';
               break;  
			}                   
        }
...
void sys_UART_TxNumber(uint8_t v_uartChannel_u8, uint8_t v_numericSystem_u8, uint32_t v_number_u32, uint8_t v_numOfDigitsToTransmit_u8)
{
    uint8_t i=0,a[10];
...
void sys_UART_TxFloatNumber(uint8_t v_uartChannel_u8, float v_floatNumber_f32)
{
...
void sys_UART_Printf(uint8_t v_uartChannel_u8, const char *argList, ...)
{
    const char *ptr;
